 ...
 *
 * `npm test` runs the `unit` project only, and CI runs `npm test` in the same
 * bare environment as `npm run build` — no DATABASE_URL, no AUTH_*. Anything
 * that needs a database therefore has to stay out of the default run, or CI
 * goes red on every commit for reasons that have nothing to do with the
 * commit. Naming a file `*.db.test.ts` is what opts it out; `npm run test:db`
 * is what runs it. See docs/testing.md.
 */
