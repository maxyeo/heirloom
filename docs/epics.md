...
| E1-T7<br>`YEO-21` | One-click restore | Restoring writes a *new* revision rather than deleting any. Append-only means restore is a copy. |
...
schema gap that gets *more* expensive the longer it waits — every date typed
...
| E5-T2<br>`YEO-42` | Upload endpoint | Auth-gated, size cap, MIME allowlist. Never trust the client's content type. |
| E5-T3<br>`YEO-43` | Image button in the editor | Upload from disk, inserted inline. No URL field. |
| E5-T4<br>`YEO-44` | Person portrait | A primary image on `individuals`, shown on the tree node and detail panel. |
...
| E7-T3<br>`YEO-53` | Download from a settings page | Plus a note on what is *not* in the file (entry bodies, images) so nobody mistakes it for a full backup. |
...
**Depends on.** E11-T1 lands *before* E1-T1, so the read route is built in the
