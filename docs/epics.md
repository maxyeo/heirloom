...
| E1-T10<br>`YEO-122` | Retire an entry, without deleting it | `pages.deleted_at` rather than a `delete`: the history, the revisions and every image reference survive, and it is reversible. Every reader filters; `lib/image-references.ts` and `lib/export-full.ts` deliberately do not. |
