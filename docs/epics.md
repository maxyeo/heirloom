...
| E5-T2<br>`YEO-42` | Upload endpoint                   | Auth-gated, size cap, MIME allowlist. Never trust the client's content type. Returns a **key**, not a storage URL — `YEO-86` made image URLs expiring, so the stable reference a body carries is a site-relative path of this app's own, resolved per request. |
