...
- **Uploads are capped at 4 MB.** Not a preference: a Vercel function
  ...
  posting. `E5-T4` built that for the portrait path
  (`lib/image-scale.ts`, `components/PortraitField.tsx`); the editor's image
  button (`E5-T3`) is the other caller and the modules are named neutrally so
  it can use them unchanged.
